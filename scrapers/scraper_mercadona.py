"""Scraper adapter for Mercadona Spain."""

from __future__ import annotations

import json
import logging
import time

try:
    from base import ScrapedListing, StoreScraper
except ImportError:
    from .base import ScrapedListing, StoreScraper

logger = logging.getLogger(__name__)

BASE_URL = "https://tienda.mercadona.es"
CATEGORIES = [
    {"slug": "leche-huevos-y-mantequilla", "name": "Lacteos y Huevos"},
    {"slug": "carne", "name": "Carne"},
    {"slug": "pescado-y-marisco", "name": "Pescado y Marisco"},
    {"slug": "fruta-y-verdura", "name": "Frutas y Verduras"},
    {"slug": "despensa", "name": "Despensa"},
    {"slug": "bebidas", "name": "Bebidas"},
    {"slug": "congelados", "name": "Congelados"},
    {"slug": "limpieza-y-hogar", "name": "Limpieza"},
]
HEADERS = {
    "User-Agent": "Mozilla/5.0",
    "Accept": "application/json,text/html;q=0.9,*/*;q=0.8",
    "Accept-Language": "es-ES,es;q=0.9,en;q=0.5",
}


def _fetch_category_products(session, category: dict, max_products: int = 50) -> list[dict]:
    import requests
    from bs4 import BeautifulSoup

    url = f"{BASE_URL}/categories/{category['slug']}"
    response = session.get(url, headers=HEADERS, timeout=20)
    response.raise_for_status()

    content_type = response.headers.get("content-type", "")
    if "application/json" in content_type:
      payload = response.json()
      items = payload.get("results", []) or payload.get("products", [])
      products = []
      for item in items[:max_products]:
          name = item.get("display_name") or item.get("name") or ""
          price = item.get("price_instructions", {}).get("unit_price") or item.get("price")
          if not name or price in (None, ""):
              continue
          products.append(
              {
                  "name": name,
                  "price": float(price),
                  "unit_price": item.get("price_instructions", {}).get("reference_price"),
                  "category": category["name"],
                  "product_id": str(item.get("id", "")),
                  "brand": item.get("brand"),
              }
          )
      return products

    soup = BeautifulSoup(response.text, "lxml")
    script = soup.find("script", {"type": "application/ld+json"})
    if not script or not script.string:
        return []
    data = json.loads(script.string)
    entries = data if isinstance(data, list) else [data]
    products = []
    for entry in entries[:max_products]:
        offers = entry.get("offers", {})
        price = offers.get("price")
        name = entry.get("name")
        if not name or price in (None, ""):
            continue
        products.append(
            {
                "name": name,
                "price": float(price),
                "unit_price": None,
                "category": category["name"],
                "product_id": str(entry.get("sku", "")),
                "brand": entry.get("brand", {}).get("name")
                if isinstance(entry.get("brand"), dict)
                else entry.get("brand"),
            }
        )
    return products


class MercadonaScraper(StoreScraper):
    country_code = "ES"
    store_id = "mercadona"
    store_name = "Mercadona"
    categories = CATEGORIES

    def scrape(self) -> list[ScrapedListing]:
        import requests

        session = requests.Session()
        listings: list[ScrapedListing] = []
        for category in self.categories:
            logger.info("Scraping Mercadona: %s...", category["name"])
            products = _fetch_category_products(session, category)
            logger.info("  Found %s products", len(products))
            for product in products:
                listings.append(
                    ScrapedListing(
                        country_code=self.country_code,
                        store_id=self.store_id,
                        store_name=self.store_name,
                        product_name=product["name"],
                        price=float(product["price"]),
                        category=product.get("category", category["name"]),
                        product_id=str(product.get("product_id", "")),
                        unit_price=product.get("unit_price"),
                        brand=product.get("brand"),
                    )
                )
            time.sleep(0.1)
        return listings
