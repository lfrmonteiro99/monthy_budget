...
class _TrendSheetContent extends StatelessWidget {
  ...
  Widget _buildStressChart(BuildContext context, S l10n) {
    ...
    return CalmCard(
      child: Column(
        ...
        children: [
          CalmEyebrow(l10n.trendStressIndex),
          ...
          Row(
            ...
            children: [
              _zoneDot(
                  context, AppColors.bad(context), l10n.stressCritical),
              const SizedBox(width: 12),
              _zoneDot(
                  context, AppColors.warn(context), l10n.stressWarning),
              const SizedBox(width: 12),
              _zoneDot(
                  context, AppColors.accent(context), l10n.stressGood),
              const SizedBox(width: 12),
              _zoneDot(
                  context, AppColors.ok(context), l10n.stressExcellent),
            ],
          ),
          const SizedBox(height: 10),
          // Zone explanations
          ...[
            (AppColors.bad(context), l10n.stressZoneCritical),
            (AppColors.warn(context), l10n.stressZoneWarning),
            (AppColors.accent(context), l10n.stressZoneGood),
            (AppColors.ok(context), l10n.stressZoneExcellent),
          ].map((entry) => Padding(
  ...
  Widget _buildExpenseTotalChart(BuildContext context, S l10n) {
    ...
    return CalmCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          CalmEyebrow(l10n.trendTotalExpenses),
          const SizedBox(height: 16),
          SizedBox(
            height: 200,
            child: BarChart(
                ...
                barGroups: List.generate(months.length, (i) {
                  return BarChartGroupData(
                    ...
                    barRods: [
                      BarChartRodData(
                        toY: totals[i],
                        // over-budget bar → bad token; otherwise accent
                        ...
                        HorizontalLine(
                          ...
                          label: HorizontalLineLabel(
                            ...
                            style: TextStyle(
                                fontSize: 10,
                                color: AppColors.ink70(context)),
                            labelResolver: (_) => l10n.trendCurrent(
                      ...
                      getTitlesWidget: (value, _) {
                        ...
                        if (idx < months.length) {
                          ...
                          return Padding(
                            padding: const EdgeInsets.only(top: 8),
                            child: Text(
                              localizedMonthAbbr(l10n, monthNum),
                              style: TextStyle(
                                  fontSize: 10,
                                  color: AppColors.ink70(context)),
                            ),
  ...
  Widget _buildExpenseCategoryChart(BuildContext context, S l10n) {
    ...
    return CalmCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          CalmEyebrow(l10n.trendExpensesByCategory),
          const SizedBox(height: 16),
          SizedBox(
            height: 200,
            child: BarChart(
  ...
  static Color _scoreColor(BuildContext context, int score) {
    if (score >= 80) return AppColors.ok(context);
    if (score >= 60) return AppColors.accent(context);
